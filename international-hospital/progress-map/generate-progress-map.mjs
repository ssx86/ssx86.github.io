  ...
  'patient:pages/patient/manage/index': [
    {
      ...
      summary: 'Natural entry was rechecked after patient automator recovery: Patient Home health-card .switch-btn -> pages/patient/manage/index. page_data showed the real patientList with maskedIdCard, cardClass, and sideLabel; element_info .navbar returned not found, .health-card and .add-btn remained visible, and console capture reported 0 logs/warnings/errors/exceptions. Card tap can switch the default patient, so read-only checks should use .detail-link for detail navigation.'
    }
  ],
  'patient:pages/patient/edit/index': [
    {
      type: 'screenshot',
      title: 'P056 visit-person detail screenshot after navbar removal',
      summary: 'Current MCP screenshot for pages/patient/edit/index after removing the copied WeChat title bar. The viewport shows the QR card, real patient name, detail rows, and fixed bottom actions without a duplicate in-page navbar.',
      path: 'evidence/patient-p056-edit-navbar-removed.png'
    },
    {
      type: 'runtime',
      title: 'P056 visit-person detail runtime evidence',
      summary: 'Natural entry from P051 .detail-link reached pages/patient/edit/index with patientId=308386498155347968. page_data showed real patient detail fields and birthDate=1990-03-07. The default-navigation page previously rendered a copied .navbar/menu capsule; after removal and patient compile, element_info .navbar returned not found, .qr-section remained visible with the patient name, and 8 second console capture reported 0 logs/warnings/errors/exceptions.'
    }
  ],
  'patient:pages/patient/add/index': [
    {
      type: 'screenshot',
      title: 'P055 add visit-person screenshot after navbar removal',
      summary: 'Current MCP screenshot for pages/patient/add/index after removing the copied WeChat title bar. The viewport shows the add-patient form fields and fixed submit area without a duplicate in-page navbar.',
      path: 'evidence/patient-p055-add-navbar-removed.png'
    },
    {
      type: 'runtime',
      title: 'P055 add visit-person runtime evidence',
      summary: 'Natural path Patient Home -> health-card .switch-btn -> P051 -> .add-btn reached pages/patient/add/index. The page uses default native navigation through navigationBarTitleText; runtime element_info confirmed the copied .navbar with back arrow/title/simulated capsule before the fix. The copied navbar WXML and related WXSS were removed. After patient compile and automator daemon recovery, natural re-entry reached pages/patient/add/index, page_data retained the expected empty form defaults, element_info .navbar returned not found, .form-section remained visible, and 8 second console capture reported 0 logs/warnings/errors/exceptions.'
    }
  ],
  'doctor:pages/patient/list/index': [
    {
