  ...
  'patient:pages/patient/add/index': [
    {
      ...
      summary: 'Earlier MCP screenshot evidence shows the add-patient form state. Current 2026-06-29 screenshot capture timed out and did not write a new file after removing the copied navbar, so the linked image is historical visual context rather than current post-fix proof.',
      path: 'evidence/patient-p055-add-current.png'
    },
    {
      type: 'runtime',
      title: 'P055 添加就诊人运行时证据',
      summary: 'Natural path Patient Home -> health-card .switch-btn -> P051 -> .add-btn reached pages/patient/add/index. The page uses default native navigation through navigationBarTitleText; runtime element_info confirmed the copied .navbar with back arrow/title/simulated capsule before the fix. The copied navbar WXML and related WXSS were removed. After patient compile and automator daemon recovery, natural re-entry reached pages/patient/add/index, page_data retained the expected empty form defaults, element_info .navbar returned not found, .form-section remained visible, and 8 second console capture reported 0 logs/warnings/errors/exceptions. Current screenshot capture timed out without writing a file, so post-fix screenshot proof is missing.'
    }
  ],
  'doctor:pages/patient/list/index': [
    {
