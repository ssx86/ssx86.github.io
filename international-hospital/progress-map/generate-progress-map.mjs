...
const evidenceByRoute = {
  ...
  'patient:pages/patient/manage/index': [
    {
      ...
      summary: 'Patient home electronic health card switch entry is visible and this route previously had natural-entry runtime evidence with real patientList, maskedIdCard, cardClass, and sideLabel. Card tap can switch the default patient, so read-only checks should use .detail-link. On 2026-06-28 the duplicate self-drawn .navbar/.back-btn/.menu-btn was removed from this default-navigation page; patient compile passed with no WXML errors. Runtime recheck is still pending because automator 9420 entered a health-check/navigation-context failure after compile, so .navbar absence and console-clean must be re-proven after MCP recovery.'
    }
  ],
  'patient:pages/patient/edit/index': [
    {
  ...
  'patient:pages/patient/add/index': [
    ...
    },
    {
      ...
      summary: '从 P051 点击 .add-btn 进入 pages/patient/add/index；初始 page_data 显示身份证/男等默认字典值、民族和地区占位；点击原生 button.btn-primary 空提交后仍停留本页、submitting=false，未写后端；民族行 .form-section .form-item:nth-child(11) 已用 element_info 和 tap 证明自然 picker 入口，但 picker 选择结果尚未通过 MCP 改变 page_data；console 采样 0 warning/error/exception。'
    }
  ],
  'doctor:pages/patient/list/index': [
    {
